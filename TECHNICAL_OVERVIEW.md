...
- **标签**：数字格式（0=中性，1=积极，2=消极）→ 统一为 `positive`、`neutral`、`negative`
- **规模**：约 9,500 个样本
- **特征**：
  - 短文本、非正式语言（典型的社交媒体特征）
  - 真实世界的噪声（标签、提及、股票代码、URL）
  - 类别不平衡：消极（64.74%）、积极（20.15%）、中性（15.11%）
...
**为什么使用宏 F1？**
- **类别不平衡**：数据集存在显著不平衡（消极：64.74%，积极：20.15%，中性：15.11%）
...
**关键观察**：
- **消极类**：高召回率（0.96），由于类别不平衡
- **中性类**：低召回率（0.36）- 最具挑战性的类别
- **积极类**：中等召回率（0.51）
...
**类别不平衡**：
- 显著不平衡（消极：64.74%，积极：20.15%，中性：15.11%）
- 模型偏向多数类（消极）
- 在少数类（中性、积极）上表现不佳
